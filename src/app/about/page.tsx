 ...
 if (!mounted) return null;
  return (
    <div className={`w-full min-h-screen ${
        theme === 'dark' ? 'bg-[#432400] text-white' : 'bg-white text-[#432400]'
      }`}>
...
<section className="w-full grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 pb-24 p-[3%] gap-5 mx-auto align-middle justify-center items-center 
  ...
  <div className={`${theme==='dark'?'bg-[#2d1400] text-slate-100':'bg-slate-100 text-slate-900'} w-full text-center flex p-4 flex-col align-middle justify-center items-center max-w-[400px] flex-1  shadow-lg`}>
...
<div className="text-sm 2xl:text-md">To provide services that are tailored to the participants preferences to enable them to have a full life in the community.</div>
  </div><div className={`${theme==='dark'?'bg-[#2d1400] text-slate-100':'bg-slate-100 text-slate-900'} w-full text-center flex p-4 flex-col align-middle justify-center items-center max-w-[400px] flex-1  shadow-lg`}>
<div className="w-16 h-16 relative"><Image src='/star.png' alt="star" fill  className="" /></div>
